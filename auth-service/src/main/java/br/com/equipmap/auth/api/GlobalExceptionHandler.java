...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ProblemResponse> apiException(ApiException exception, HttpServletRequest request) {
        ProblemResponse response = exception.toProblemResponse(traceId(request));
        return ResponseEntity.status(exception.statusCode()).body(response);
    }

    @ExceptionHandler(AccessDeniedException.class)
    ResponseEntity<ProblemResponse> accessDenied(AccessDeniedException exception, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.FORBIDDEN)
                .body(ProblemResponse.of(403, "FORBIDDEN", "Access denied", List.of(), traceId(request)));
    }
