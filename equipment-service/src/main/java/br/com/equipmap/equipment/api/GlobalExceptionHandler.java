...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ProblemResponse> apiException(ApiException exception, HttpServletRequest request) {
        return ResponseEntity.status(exception.statusCode())
                .body(exception.toProblemResponse(traceId(request)));
    }

    @ExceptionHandler(DataIntegrityViolationException.class)
    ResponseEntity<ProblemResponse> dataIntegrity(DataIntegrityViolationException exception, HttpServletRequest request) {
        ConflictException conflict = new ConflictException("Equipment data conflicts with an existing record");
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(conflict.toProblemResponse(traceId(request)));
    }
    ...
    ResponseEntity<ProblemResponse> validation(MethodArgumentNotValidException exception, HttpServletRequest request) {
        ...
        return ResponseEntity.badRequest()
                .body(ProblemResponse.of(400, "BAD_REQUEST", "Validation failed", details, traceId(request)));
    }

    @ExceptionHandler(Exception.class)
    ResponseEntity<ProblemResponse> generic(Exception exception, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(ProblemResponse.of(500, "INTERNAL_SERVER_ERROR", "Unexpected error", List.of(), traceId(request)));
    }
