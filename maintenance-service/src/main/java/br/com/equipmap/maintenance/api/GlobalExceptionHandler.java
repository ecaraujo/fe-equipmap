...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ProblemResponse> optimistic(ObjectOptimisticLockingFailureException exception, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.CONFLICT).body(ProblemResponse.of(409, "CONFLICT", "Maintenance was modified concurrently", List.of(), traceId(request)));
    }

    @ExceptionHandler(MethodArgumentNotValidException.class)
    ResponseEntity<ProblemResponse> validation(MethodArgumentNotValidException exception, HttpServletRequest request) {
        List<ErrorDetail> details = exception.getBindingResult().getFieldErrors().stream().map(error -> new ErrorDetail(error.getField(), error.getDefaultMessage())).toList();
    ...
    ResponseEntity<ProblemResponse> generic(Exception exception, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ProblemResponse.of(500, "INTERNAL_SERVER_ERROR", "Unexpected error", List.of(), traceId(request)));
    }
