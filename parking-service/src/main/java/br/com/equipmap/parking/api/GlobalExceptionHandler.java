...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler({DataIntegrityViolationException.class, CannotCreateTransactionException.class})
    ResponseEntity<ProblemResponse> conflict(RuntimeException exception, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(ProblemResponse.of(409, "CONFLICT", "Parking operation conflicts with existing data or concurrent execution", List.of(), traceId(request)));
    }
